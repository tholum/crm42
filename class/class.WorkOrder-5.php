...
var $total_est_day;
	...
	function showproductname($order_id=''){
			...
			while($row=$this->db->fetch_array($result)){ ?>
				...
				<div id="div_<?php echo $row[workorder_id];?>">
					...
					<?php if( $check_num > 0 ){ ?>
					    <div class="form_main" id="wo_task<?php echo $row[workorder_id].'rework';?>">
							<div id="task_area" class="small_text" style="background-color:#CCCCCC;">
								
							<a href="javascript:void(0);" id="task_link<?php echo $row[workorder_id].'rework';?>" 
							   onclick="javascript:document.getElementById('task_form<?php echo $row[workorder_id].'rework';?>').style.display='';
										workorder.addTodo('WORK_ORDER',
														  ...
														  {preloader: 'prl',
														  onUpdate:function(response,root){
														    document.getElementById('task_form<?php echo $row[workorder_id].'rework';?>').innerHTML = response;
														    showCalender();
														  }});">add Todo</a><br />
									<?php  $product_id = $_POST["hide_pro123"];
									
											if($_REQUEST['Save_WORK_ORDER']=='Add this Task'){
														echo $this->addTodo('WORK_ORDER','order.php','work_order_id',$order_id,$row['workorder_id'],'server');
												   }?>
										
								<div class="form_bg" style="display:none;" id="task_form<?php echo $row[workorder_id].'rework';?>"></div>																					  									<div id="tab<?php echo $row['product_id'];?>"><?php echo $this->show_title($order_id,$row['product_id'],'WORK_ORDER');?></div>
								
								<b>Rework : </b>
								<?php echo $this->FlowChartDiv("work order", $row["workorder_id"], "rework order");?>
								...
								<?php if($item==''){?>
								   <input type="button" value="Add"  style="size:auto"
								           onclick="workorder.GenerateRework('server',
														...
														<?php /*?>workorder.ShowRework('<?php echo $order_id; ?>',
																			 '<?php echo $product_id; ?>',
																			 {preloader:'prl',
																			  target:'rework<?php echo $product_id; ?>'});<?php */?>
															}});"/><?php }
	...
	function addTodo($module='',$profile_page='',$module_id='',$order_id='',$product_id='',$type='',$rework=''){
		ob_start();
			...
			?>
			<a href="javascript:void(0);" onClick="document.getElementById('task_link<?php echo $product_id.''.$rework;?>').style.display=''; 
												   document.getElementById('task_form<?php echo $product_id.''.$rework;?>').style.display='none';
...
<?php */ 
  ...
  function FlowChartDiv( $module_name , $module_id , $rework_order='' ){

	  $module_name_true = $module_name;
      $module_name = str_replace( ' ' , '' , $module_name );
	  $rework_order_true = $rework_order;
	  $rework_order = str_replace( ' ' , '' , $rework_order );
      ob_start();
	  $global_task = new GlobalTask();
      ?>
      <div id="flowcharttask_<?php echo $module_name . '_' . $module_id . '_' . $rework_order;?>">
        <a href="javascript:void(0);" onClick="javascript: workorder.check_shipdate('<?php echo $_REQUEST[order_id];?>',
																					{onUpdate:function(response,root){
																						if(response){
																						  $('#flowcharttask_add_<?php echo $module_name . '_' . $module_id . '_' . $rework_order;?>').show();
																						}
																						else{
        ...
        <div style="display: none;" id="flowcharttask_add_<?php echo $module_name . '_' . $module_id . '_' . $rework_order;?>">
            <?php echo $global_task->AddFlowChartTask($module_name_true, $module_id , '' , '' , "$('#flowcharttask_add_" . $module_name . '_' . $module_id . '_' . $rework_order . "').hide()" , $_REQUEST[order_id] , $rework_order_true);?>
        </div>

		<div id="flowcharttask_options_<?php echo $module_name . '_' . $module_id . '_' . $rework_order;?>">
			<?php echo $global_task->displayByModuleId($module_name_true, $module_id, "flowcharttask_options_" . $module_name . '_' . $module_id . '_' . $rework_order,'',$_REQUEST[order_id], $rework_order_true);?>
		</div>
      </div>
