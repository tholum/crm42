<?php
  ...
  */
  ...
  class payflow {
    ...
    var $test_mode = PAYFLOW_MODE; // 1 = true, 0 = production
    
    function payflow() {
      
      $this->vendor = PAYFLOW_VENDOR;
      $this->user = PAYFLOW_USER;
      $this->partner = PAYFLOW_PARTNER;
      $this->password = PAYFLOW_PASSWORD;
    ...
    //function sale_transaction_full( $card_number , $card_expire , $amount , $firstname , $lastname )
    
    function get_data_array( $first , $last , $street , $city , $state , $zip , $cvv , $comment ){
        $data_array = array('comment1' => $comment,
                        'firstname' => $first,
                        'lastname' => $last,
                        'street' => $street,
                        'city' => $city,
