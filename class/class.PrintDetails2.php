 ...
 class PrintDetails{
    ...
    function search_screen(){
	 ...
	 function order_summary(){
		 ob_start();
		 $sql_type = "SELECT distinct name as inventory_name,inventory_id,type_id FROM ".TBL_INVENTORY_DETAILS." WHERE type_id = '5'";
		 //echo $sql_type;
		 $result_type = $this->db->query($sql_type,__FILE__,__LINE__); 
				...
				if( $this->db->num_rows($result_size) > 0 ){
					while($row_size = $this->db->fetch_array($result_size)){
					  ...
					  if( $row_size[module] == 'work order' ){
					    $sql_sub = "select gp_id from ".erp_PRODUCT_ORDER." where workorder_id = '$row_size[workorder_id]'";
						//echo $sql_sub;
						...
						while( $row_inch = $this->db->fetch_array($result_inch) ){
							...
							switch($size_array[1]){
							   case 'XS':
								 $size_array[0] *=  $row_inch[quantity] ;
							   break;
							   case 'S':
								 $size_array[0] *=  $row_inch[quantity] ;
							   break;
						 ...
						 //echo $sql_r.'<>'.$row_type['inventory_name'];
	...
	function hide_value($check='',$hidden_value='',$g_id=''){
	   ob_start();
	   switch($check){
		  case 'insert' :
			$id=$hidden_value;
			...
			for($i=1;$i<$len;$i++){
				$d1=explode("_",$seperate[$i]);
                                if( count( $d1 ) > 1 ){
                                    ...
                                    $insert_sql_array[assign_fct_id] = $d1[5];
			...
			$sql_in = "update ".erp_GROUP." set total_inch = '$this->total_inch' where group_id = '$gd_id'";
	 ...
	 function work_orders($color_profile='',$order_id='',$inventory_id='',$due_date='',$thru_date='',$inventory_name='',$g_id='',$module_id='',$inches='',$flag='',$checked='',$grouped=''){
		ob_start();
		$sql_type = "SELECT group_id FROM ".erp_USERGROUP." WHERE group_name = 'Print'";
		$result_type = $this->db->query($sql_type,__FILE__,__LINE__);
		$row_type = $this->db->fetch_array($result_type);
		
        $sql_print = "SELECT a.chart_assign_id, a.due_date,a.module_id,a.module, d.*, e.fabric, e.profile_JV5, e.product_id, f.name, f.inventory_id FROM ".erp_ASSIGN_FCT." a, ".erp_GLOBAL_TASK." b, ".erp_ORDER." d, ".erp_WORK_ORDER." e, ".TBL_INVENTORY_DETAILS." f WHERE a.flow_chart_id = b.global_task_id and a.task_status = 'Active' and b.department_id = '$row_type[group_id]' and a.module IN ('work order','rework order') and a.product_id = d.order_id and d.order_id = e.order_id and a.module_id = e.product_id and e.fabric = f.inventory_id ";
	   ...
	   if($module_id){
		...
		echo $sql_print;
		if($flag == '1'){
		$this->total_inch_c = ($inches*0.027778);	
		...
		<input type="button" name="creategroup" id="creategroup" value="CreateGroup" 
							 onclick="javascript:print.create_group(
							                           {preloader:'prl',
													    onUpdate:function(response,root){
														print.hide_value('check',
			  ...
			  while( $row_order = $this->db->fetch_array($result_order) ){
				  ...
				  if( $row_order[module] == 'work order' ){
					 ...
					 while( $row_inch = $this->db->fetch_array($result_inch) ){
							 ...
							 switch($inch_array[1]){
							   ...
							   case 'XS':
								 $inch_array[0] *=  $row_inch[quantity] ;
							   break;
							   case 'S':
								 $inch_array[0] *=  $row_inch[quantity] ;
							   break;
						 ...
						 $inch += $inch_array[0];
						//echo 'si'.$row_inch[size].'quan'.$row_inch[quantity].'ma'.$inch_array[0];
					 }
					 //echo '<>'.$inch;
				 } else {
					...
					<td>
					  <?php if( $inventory_name != '' and $inventory_name == $row_order[name] ){ ?>
					           ...
					           <input type="checkbox" name="group_id" id="<?php echo $row_order[module_id].'_'.$row_order[order_id].'_'.$row_order[module].'_'.$row_order[chart_assign_id];?>"
                                                          value="<?php echo $row_order[module_id].'_'.$row_order[order_id].'_'.str_replace(',', "", $row_order[name]).'_'.$inch.'_'.$row_order[module].'_'.$row_order[chart_assign_id].'_'.$row_order[inventory_id].'_'.$rework_id; ?>" 
							 			onclick="javascript:
                                        if(this.checked){
				...
				<?php
					...
					<td><?php if($inch!=''){
								   if( $row_order[module] == 'work order' ){
								       echo ($inch*0.027778);
							       } elseif( $row_order[module] == 'rework order' ) { echo ($inch*0.027778);  }
							  } else { echo '#'; } ?></td>
	 ...
	 function total_inches($total='',$inch='',$check=''){
	 	ob_start();
		$inch = ($inch*0.027778);
		if($check == ''){
					...
					?>
						...
						<td>
                             <span id="inches_box_<?php echo $row_type['id']; ?>">
						       <?php echo $this->returnLink(($row_type[inches]*0.027778),'inches_box_'.$row_type[id],'inch',$row_type[id]); ?>
                             </span>
                        </td>
						<td>
	  ...
	  function groups(){
		  ...
		  <table id="search_table" class="event_form small_text" width="100%">
			...
			<tbody>
			  <?php
                          //echo erp_GROUP;
			  $sql = "SELECT distinct group_id,inventory_name,total_inch,created FROM ".erp_GROUP."  ORDER BY group_id ASC";
			  $result = $this->db->query($sql,__FILE__,__LINE__);
			  $total_rows=$this->db->num_rows($result);
				 ...
				 <tr>
					...
					<td><?php echo ( $row['total_inch'] * 0.027778 );?></td>
	 ...
	 function delete_group($group_id)
	{
	  ...
	  function showTextBox($choice='',$roll_id=''){
		 ...
		 $sql_type= "select * from ".TBL_FABRIC_ROLLS." where id = '$roll_id'";
			...
			$result_type=$this->db->query($sql_type,__FILE__,__LINE__);
			$row_type=$this->db->fetch_array($result_type);
			$yards = ($row_type[inches] * 0.027778);
		 switch($choice){
			case 'inch':
		     ?>
