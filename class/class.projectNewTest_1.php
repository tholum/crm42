...
var $user_id;
	...
	function showProjectSearch(){
		...
		<form name="<?php echo $formName;?>" method="post" action="">
			<table class="table" width="35%">
				...
				<tr>
					<td>
						<select name="cust_id" id="cust_id" onChange="javascript: document.getElementById('customer_id').value='';
											...
											project_new.searchProjectList(document.getElementById('customer_id').value,
																		  document.getElementById('user_id').value,
																		  document.getElementById('chk_unread').value,
																		  document.getElementById('chk_important').value,
																		  document.getElementById('chk_unclaimed').value,
																		  {target:'show_search_result',preloader:'prl'}); "></select> 
					...
					<td>
						<select name="u_id" id="u_id" onChange="javascript: document.getElementById('user_id').value='';
											...
											document.getElementById('user_id').value.length-1);
				...
				<tr>
					<td><input type="checkbox" name="chk_unread" id="chk_unread" style="vertical-align:middle; width:0;"
						onclick="javascript:if(document.getElementById('chk_unread').checked==true){
												document.getElementById('chk_unread').value = 'unread';
											}
											else{ 
												document.getElementById('chk_unread').value = 'read';										
											}
											project_new.searchProjectList(document.getElementById('customer_id').value,
																		  document.getElementById('user_id').value,
																		  document.getElementById('chk_unread').value,
																		  ...
																		  onUpdate: function(response,root){
																			document.getElementById('show_search_result').innerHTML=response;}});"/>
																		Show Unread Messages</td>
				</tr>
				<tr>
					<td><input type="checkbox" name="chk_important" id="chk_important" style="vertical-align:middle; width:0;" 
						onclick="javascript:if(document.getElementById('chk_important').checked==true){
												document.getElementById('chk_important').value = 'important';
											}	
											else{ 
												document.getElementById('chk_important').value = '';											
											}
				...
				<tr>
					<td><input type="checkbox" name="chk_unclaimed" id="chk_unclaimed" style="vertical-align:middle; width:0;" 
						onclick="javascript:if(document.getElementById('chk_unclaimed').checked==true){
												document.getElementById('chk_unclaimed').value = 'unclaimed';
											}
											else{ 
												document.getElementById('chk_unclaimed').value = 'claimed';
											}
											project_new.searchProjectList(document.getElementById('customer_id').value,
																		  document.getElementById('user_id').value,
																		  document.getElementById('chk_unread').value,
	...
	function searchProjectList($customer_id='',$user_id='',$unread='',$imp_type='',$unclaimed='',$project_id=''){
		ob_start();

/*		if($unclaimed == 'unclaimed' || $unclaimed != 'claimed'){
	    	$sql = "select distinct a.project_id,a.title,a.importance_type_id,a.due_date,f.importance_type_value from ".PROJECT." a ,".IMPORTANCE_TYPE." f where a.user_id = '0' and a.importance_type_id = f.importance_type_id";
	...
	if($unclaimed=='claimed'){	*/
			...
			$sql="select distinct a.project_id,a.title,a.due_date,a.importance_type_id,d.user_id,d.first_name as user_first_name,d.last_name as user_last_name";
			...
			$sql .= " from ".PROJECT." a, ".TBL_USER." d, ".USER_IN_PROJECT." e ";
			
			if($customer_id){
				$sql .= " ,".TBL_CONTACT." b, ".CONTACT_IN_PROJECT." c";		
			}
			
			//$sql .= " where a.importance_type_id = f.importance_type_id and a.user_id = d.user_id";
			
			//$sql .= " where a.user_id = d.user_id and a.complete <> '100' and a.project_id NOT IN ( SELECT DISTINCT connected_project_id FROM connected_project WHERE connected_project_id NOT IN ( SELECT project_id FROM connected_project)) ";
			
			$sql .= " where a.user_id = d.user_id and a.complete <> '100' and a.due_date > '".date('Y-m-d')."'";
			
			if($customer_id){
				$sql .= " and a.project_id = c.project_id and b.contact_id = c.contact_id and c.contact_id = '$customer_id'";
			}
			
			if($user_id != 'unclaimed' && $user_id != ''){
	...
	function showSlider($result){
		ob_start();
		while($row=$this->db->fetch_array($result)){ ?>
			<div id="div_<?php echo $row['project_id']; ?>" class="event_form small_text">
				<h3>
				<table width="100%">
				<tr>
					<td width="4%">
					...
					<?php	if($row['user_first_name']!=''){ ?>
								<span style="color:#666666"><?php echo $row['user_first_name'].' '.$row['user_last_name']; ?></span>
							<?php }
							else{
								echo 'UNCLAIMED';
							} ?>
				...
				<div style="width:739px">
					<table id="display_search" class="event_form small_text" width="100%">
						<tbody><?php echo $this->TaskForProject('','','','','','','',1,'PROJECT',$row['project_id']);?></tbody>
					</table>
					<div id="subProject" style="margin-left:30px">
						<?php 
...
/*		$x = floor($days/7); 
	...
	function showProjectDetails($runat,$project_id=''){
		...
		switch($runat){
			case 'local':
				...
				<table class="table" width="30%" align="left">
					<tr>
						<th>Customer</th>
						<th>User</th>
					</tr>
					<tr>
						...
						<td>
							<select name="edit_u_id" id="edit_u_id" onChange="javascript: document.getElementById('edit_user_id').value='';
											...
											var id = <?php echo $row_user['user_id']; ?>;
											if(id == ''){
												project_new.showProjectDetails('server',
																		  document.getElementById('edit_customer_id').value,
																		  document.getElementById('edit_user_id').value,
																		  {target:'show_search_result',preloader:'prl'});
											}
											if(document.getElementById('edit_user_id').value == '<?php echo $row_user['user_id']; ?>'){
												project_new.showProjectDetails('server',
																			   document.getElementById('edit_customer_id').value,
					...
					<tr>
						<td colspan="2"><input type="text" name="project_name" id="project_name" style="width:310px;" value="<?php echo $row['title']; ?>"/></td>
					</tr>
					<tr>
						<td>Comment</td>
						<td>&nbsp;</td>
					...
					<tr>
						<td>When's it due : </td>
						<td><input type="text" name="due_dt" id="due_dt" readonly="true" style="width:auto" autocomplete='off' value="<?php echo $row['due_date']; ?>"/>
					...
					<tr>
						<td><a href="javascript:void(0);">Advance</a></td>
						<?php if($project_id !=''){ ?>
		...
		case 'server':
			extract($_POST);
				...
				$insert_sql_array[user_id] = $edit_user_id;	
			...
			case 'local':
				...
				<input type="hidden" name="task_id" id="task_id" value="<?php echo $task_id; ?>" />
 				...
 				<table class="table" width="30%" align="left">
					...
					<tr>
						<td colspan="2"><input type="text" name="task_name" id="task_name" style="width:310px;" value="<?php echo $row['title']; ?>"/></td>
					</tr>
					<tr>
						<td>Comment</td>
						<td>&nbsp;</td>
	...
	function TaskForProject($module_id='', $cat_id='', $completed='', $upcomming='', $assigned='', $tagModule='', $tagModule_id='',$listonly=0,$assigned_to_module='',$assigned_to_module_id='',$assigned_to='',$module_name='',$include_all_visible=0)
	{
		...
		if($this->db->num_rows($record)>0){
			$this->showTaskForProject($record,$listonly,$assigned_to_module,$assigned_to_module_id,$assigned_to,$upcomming,$completed);
		} 
	...
	function showTaskForProject($record,$listonly=0,$assigned_to_module='',$assigned_to_module_id='',$assigned_to='',$upcomming='',$completed='',$showday='')
	{
		$i = 1;
		while($row=$this->db->fetch_array($record))	{
			if($i%2==0){ ?>
	...
	function updateNoteStatus($runat,$type_id='',$note_id='', $user_id='', $desc='',$type='',$module='',$target=''){
		...
		switch($runat) {
			case 'local':
				...
				?>
				<div class="prl">&nbsp;</div>
				<div id="lght12" style="margin-top:-200px;!important;">
					<div id="lightbox" style="position:absolute;!important;" >		
						<div style="background-color:#ADC2EB;" class="ajax_heading">
							...
							<div id="TB_closeAjaxWindow">
								<a href="javascript:void(0);" onclick="javascript: document.getElementById('lght12').style.display='none';
																				   project_new.updateNoteStatus(
																						'server',
																						'<?php echo $type_id; ?>',
																						'<?php echo $note_id; ?>',
																						'',
																						...
																						{preloader:'prl',
																						 onUpdate: function(response,root){
																						 project_new.returnNoteLink(
																							'<?php echo $type_id; ?>',
																							'<?php echo $note_id; ?>',
																							'read',
																							{preloader:'prl',
																							 onUpdate: function(response,root){
																							     document.getElementById('notes<?php echo $type_id; ?>').innerHTML=response;
																							 }});												 
																						}});">
				...
				<?php
		 		break;
			case 'server':
				$sql_notes = "Select * from ".TBL_NOTE_STATUS." where module_id='$type_id' and note_status='unread'";
				//echo $sql_notes;
				$result_notes=$this->db->query($sql_notes,__FILE__,__lINE__);	
                             			 ...
                             			 <input type="button" name="add" value="ADD" style="width:70px;" 
                                                                 onclick="javascript: if(document.getElementById('desc').value == ''){
																					   ...
																					   var project_id = document.getElementById('project_id').value;												 																						project_new.updateNoteStatus(
																								...
																								{preloader:'prl',
																								 onUpdate: function(response,root){
																								 document.getElementById('notes'+project_id).innerHTML=response;
																								}}); }" /></td>                                           
								</tr>
							 </table>
                </div>
	...
	function showSubProject($project_id=''){
		ob_start();

		$sql_sub_project = "Select * from ".CONNECTED_PROJECT." a, ".PROJECT." b where a.connected_project_id = b.project_id and a.project_id = '$project_id'";
		$result_sub_project=$this->db->query($sql_sub_project,__FILE__,__lINE__);
		while($row_sub_project=$this->db->fetch_array($result_sub_project)){ ?>
			<?php echo $row_sub_project['title']; ?>
