...
<div style='z-index: 21;position: relative;left: -200px;top: -150px;width: 400px; height: 300px;background: white;border-top-left-radius: 20px;border-top-right-radius: 20px;' >
    ...
    <div class="payment_all" style="width: 100%;text-align: center;font-size: 12px;font-weight: bold;display: none;"><button style="width: 75px;" onclick="global_task.livex_call( 'down_payment_post', { clockkey: $('#payment_clockkey').val() , module_name: '<?php echo $module_name;?>' , module_id: '<?php echo $module_id; ?>' , chart_assign_id: '<?php echo $chart_assign_id; ?>', selected_option: '<?php echo $selected_option; ?>' , ccid: $('#payment_creditcard').val() , contact_id: '<?php echo $contacts[0];?>' , target: '<?php echo $args[4] ?>' ,payment_type : systask_payment_type ,  global_task_status_id: '<?php echo $selected_option; ?>' }  , {target: 'payment_result'})">Submit</button></div>
