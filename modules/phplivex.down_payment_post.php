...
if(key_exists("error", $card) == false ){
    ...
    if( round( $bill_amount , 2 ) > round( $already_payed, 2) ){
        ...
        if( $cr["RESPMSG"] == 'Approved' ){
            ...
            $return .= "<div style='width: 100%;background: red;' >An Error Occured<br/>" .  $cr["RESPMSG"] . "</div>";
            //$return .= "<br>\sale_transaction(" . $card["ccnum"] . ", " . $card["expiration"] . ", $bill_amount,'USD', \$data_array , " . $card["name"] . ");";
        }
    } else {
