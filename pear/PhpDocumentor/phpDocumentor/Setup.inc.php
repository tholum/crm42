...
class phpDocumentor_setup
{
    ...
    function readConfigFile($file)
    {
        ...
        } else
        {
            if ('/var/www' != '@'.'DATA-DIR@')
            {
                $configdir = str_replace('\\','/', '/var/www/PhpDocumentor') . PATH_DELIMITER . 'user' . PATH_DELIMITER;
            } else {
    ...
    function parseIni()
    {
        ...
        if ('/var/www' != '@'.'DATA-DIR@')
        {
            $options = phpDocumentor_parse_ini_file(str_replace('\\','/', '/var/www/PhpDocumentor') . PATH_DELIMITER . 'phpDocumentor.ini',true);
            phpDocumentor_out("   (found in " . '/var/www/PhpDocumentor' . PATH_DELIMITER . ")...\n");
        } else {
