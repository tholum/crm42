							...
							case 'local':
									...
									<table class="subject_header">
									  ...
									  <?php 
										...
										?><input type="text" name="username" id="username" value="<?php echo $row_em[username]; ?>" 
										style="border:none; background:none" readonly="true"/>
								  		<span onclick="javascript: document.<?php echo $FormName; ?>.username.readOnly=false;
																  document.<?php echo $FormName; ?>.username.style.background='#fff';">Edit</span>
										<?php } ?>
									  <ul id="error_list"><li><span id="span_person_username"></span></li></ul></td>
									  </tr>
									 ...
									 <input type="password" name="password" id="password" size="30" />
									  ...
									  <?php }
											...
											{
									  ?>
									  <input type="hidden" name="password" id="password" value="<?php echo $row_em[password]; ?>" >
									  <ul id="error_list"><li><span id="span_person_password"></span></li></ul>
									  <?php } ?>
									  
									  <tr>
									  <th><h2>Email </h2></th>
									  <td><input type="text" name="em_email" id="em_email" size="30" value="<?php echo $row_em[email] ?>" />
									  <ul id="error_list"><li><span id="span_person_email"></span></li></ul></td>
									  </tr>
									  </table>
									  </td></tr>
...
function Delete_Availability($event_id,$unavailable_date,$start_time,$end_time,$action) {
	...
	function mtborahContactScreenCustom($contact_id){
		ob_start();
		$sql = "select * from ".MTBORAH_CONTACTSCREEN_CUSTOM." where contact_id = '$contact_id'";
		$result = $this->db->query($sql,__FILE__,__lINE__);		
		$row=$this->db->fetch_array($result);
		$FormName = 'frm_mtborah_contact_screen';
		...
		<form name="frm_mtborah_contact_screen" action="" method="post" >
		...
		<td><input name="account_type" id="account_type" type="text" value="<?php echo $row[account_type];?>" /></td>
		</tr>
		...
		<td><select name="csr" id="csr" type="text" >
			...
			<option value="<?php echo $row1[user_id];?>" <?php if($row[csr] == $row1[user_id]) echo 'selected="selected"';?> >
				<?php echo $row1[first_name].' '.$row1[first_name];?>
			</option>
			<?php } ?>
			</select>
		</td>
		</tr>
		<tr><td colspan="2"><a href="javascript:void(0);" onclick="javascript:
								if(<?php echo $ValidationFunctionName?>()){
									em.savemtborahCustomFields(
										'<?php echo $row[contact_screen_custom_id];?>',
										'<?php echo $contact_id;?>',
										document.frm_mtborah_contact_screen.account_id.value,
										document.frm_mtborah_contact_screen.account_type.value,
										document.frm_mtborah_contact_screen.csr.value,
										{preloader:'prl'});
									}">Save</a></td></tr>
	...
	function savemtborahCustomFields($contact_screen_custom_id='',$contact_id='',$account_id='',$account_type='',$csr=''){
		ob_start();
		...
		if($contact_screen_custom_id) $this->db->update(MTBORAH_CONTACTSCREEN_CUSTOM,$sql_array,'contact_screen_custom_id',$contact_screen_custom_id);		
		else $this->db->insert(MTBORAH_CONTACTSCREEN_CUSTOM,$sql_array);		
